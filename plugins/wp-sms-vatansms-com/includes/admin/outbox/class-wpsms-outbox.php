...
class Outbox_List_Table extends \WP_List_Table {
	...
	function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'recipient':
				$html = '<details>
						  <summary>' . __( 'Detay görmek için tıklayın...', 'wp-sms' ) . '</summary>
			...
			case 'status':
				if ( $item[ $column_name ] == 'success' ) {
					return '<span class="wp_sms_status_success">' . __( 'Başarı', 'wp-sms' ) . '</span>';
				} else {
					return '<span class="wp_sms_status_fail">' . __( 'Başarısız', 'wp-sms' ) . '</span>';
				}
			default:
	...
	function column_sender( $item ) {
		...
		//Build row actions
		$actions = array(
			'resend' => sprintf( '<a href="?page=%s&action=%s&ID=%s">' . __( 'Yeniden gönder', 'wp-sms' ) . '</a>', $_REQUEST['page'], 'resend', $item['ID'] ),
			'delete' => sprintf( '<a href="?page=%s&action=%s&ID=%s">' . __( 'Sil', 'wp-sms' ) . '</a>', $_REQUEST['page'], 'delete', $item['ID'] ),
		);
	...
	function get_columns() {
		$columns = array(
			...
			'sender'    => __( 'Gönderici', 'wp-sms' ),
			'date'      => __( 'Zaman', 'wp-sms' ),
			'message'   => __( 'Mesaj', 'wp-sms' ),
			'recipient' => __( 'Alıcı', 'wp-sms' ),
			'response'  => __( 'Response', 'wp-sms' ),
			'status'    => __( 'Durum', 'wp-sms' ),
		);
	...
	function process_bulk_action() {
		...
		if ( 'resend' == $this->current_action() ) {
			...
			if ( is_wp_error( $error ) ) {
				...
				echo '<div class="notice notice-success is-dismissible"><p>' . __( 'SMS başarıyla gönderildi.', 'wp-sms' ) . '</p></div>';
			}
			$this->data  = $this->get_data();
			$this->count = $this->get_total();
